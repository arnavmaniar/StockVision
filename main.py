...
def main():
    ...
    if st.button('Run'):
        ...
        for ticker in tickers:
            ...
            if df.empty:
                st.write(f"No data found for {ticker}. Skipping...")
                continue
            ...
            if train_df.empty:
                st.write(f"No training data available for {ticker}. Skipping...")
                continue
            
            models = train_models(train_df)
            
            # model metrics
            st.write(f"Model Performance for {ticker} on Training Data:")
